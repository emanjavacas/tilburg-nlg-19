...
class UnsmoothedLM(object):
    ...
    def _generate_next(self, prefix, temperature=1.0):
        ...
        try:
            ...
            probs = utils.apply_temperature(np.array(probs), temperature)
            sampled = np.random.multinomial(1, probs, 1).argmax()
        ...
        for *prefix, target in ngrams(text, self.order + 1, self.pad_token):
            ...
            try:
                ...
                if score_entropy:
                    ...
                    scores.append(utils.entropy(probs, len(self.vocab)))
                else:
